...
// Returns approved templates grouped by name, each with its available
// (approved) language variants and normalized header/body/footer/buttons.
async function listApprovedTemplates({ forceRefresh } = {}) {
  ...
  const approved = raw.filter((t) => t.status === 'APPROVED');
