...
// Admin-only hard delete. Messages and wa_id_history cascade with the
// conversation row; audit rows are kept intentionally — they are the trail
// that the deletion happened. Returns the storage keys of archived media so
// the caller can clean up the blobs best-effort.
async function deleteConversation(conversationId) {
  const keys = await sql`
    SELECT storage_key FROM messages
    WHERE conversation_id = ${conversationId} AND storage_key IS NOT NULL
  `;
  const rows = await sql`DELETE FROM conversations WHERE id = ${conversationId} RETURNING id`;
  return { deleted: !!rows[0], storageKeys: keys.map((r) => r.storage_key) };
}

// An agent manually replying on a bot-mode conversation implicitly takes it
// over — the bot must not keep answering behind the agent's back. No-op when
// the conversation is already in human mode.
async function takeOverOnAgentReply(conversationId, adminUserId) {
  const rows = await sql`
    UPDATE conversations
    SET mode = 'human',
        status = CASE WHEN status = 'closed' THEN status ELSE 'open' END,
        assigned_to = COALESCE(assigned_to, ${adminUserId}),
        handoff_requested_at = COALESCE(handoff_requested_at, now()),
        updated_at = now()
    WHERE id = ${conversationId} AND mode = 'bot'
    RETURNING *
  `;
  return rows[0] || null;
}
...
async function recordOutboundMessage({
  ...
  const rows = await sql`
    INSERT INTO messages (
      ...
      latitude, longitude, location_name, location_address, contacts_data,
      reaction_emoji, reacted_message_wa_id, context_message_wa_id, media_status, media_id
    )
    VALUES (
...
async function updateMessageStatusByWaId(waMessageId, status, errors) {
  if (errors && errors.length) {
    // Keep Meta's failure reasons so the dashboard can show WHY a message
    // never reached the customer (24h window, unsupported media, etc.).
    const rows = await sql`
      UPDATE messages
      SET status = ${status},
          raw_payload = COALESCE(raw_payload, '{}'::jsonb)
            || jsonb_build_object('delivery_errors', ${JSON.stringify(errors)}::jsonb)
