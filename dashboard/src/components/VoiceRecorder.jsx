...
export default function VoiceRecorder({ onSend, onClose }) {
  ...
  const { state, duration, error, result, start, stop, cancel, reset } = useVoiceRecorder();
  ...
  if (state === 'recording') {
    return (
      <div className="flex items-center justify-between gap-3 rounded-xl border border-danger/30 bg-danger-soft px-4 py-3">
